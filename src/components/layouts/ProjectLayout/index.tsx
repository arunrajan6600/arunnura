...
const Component: React.FC<ComponentProps> = (props) => {
    ...
    const dateTimeAttr = dayjs(date).format('MM-DD-YYYY HH:mm:ss');
            ...
            <main id="main" className="sb-layout sb-project-layout">
                    ...
                    <div className="max-w-5xl mx-auto">
                        <header className="mb-10 sm:mb-16">
                            ...
                            <div className="md:flex md:justify-between">
                                ...
                                <h4 className="md:max-w-lg md:flex-grow">{title}</h4>
                            </div>
                        </header>
                        {description && <div className="text-sm leading-normal lowercase max-w- -md mx-auto mb-10 sm:mb-16">{description}</div>}
                        {media && (
                ...
                {(prevProject || nextProject) && (
                    <nav className="sb-project-nav px-4 sm:px-8 mt-12 mb-20">
                        <div className="max-w-xl mx-auto grid gap-x-6 gap-y-12 md:grid-cols-2 lg:gap-x-8">
                            {prevProject && <ProjectNavItem project={prevProject} label="Previous" />}
                            {nextProject && <ProjectNavItem project={nextProject} label="Next" />}
                        </div>
                    </nav>
                )}
